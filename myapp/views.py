...
def cart_p(request):
    ...
    for i in  cart1:
        ...
        for j in product1:
            ...
            # for i in cart1:
            cart2[i.id]=a
    return render(request,'cart.html',{'products':cart2,'uid':uid,'total':total,'count':count})

# @login_required(login_url='/')
def addtocart(request):
    if request.method == 'POST':
        pid = request.POST['product_id']
        chekccontity = cart.objects.filter(userid = request.session.get('uid'), product_id = pid)
        if chekccontity:
            pid = request.POST['product_id']
            c = cart.objects.get(userid = request.session.get('uid'),product_id = pid)
            if c: 
                contity12 = request.POST['contity']
                if c.contity < int(contity12):
